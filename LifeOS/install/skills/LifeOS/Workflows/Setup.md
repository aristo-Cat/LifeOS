...
6. **LinkUser** — `bun Tools/LinkUser.ts` → relocate/link the config tree into the harness tree (3-branch logic ported from the install engine; directory junction on Windows; copy fallback only if links are blocked). Config root keeps its canonical name.
...
7. **Enhancements menu** — present the seven optional components; let the user (or their AI, from context) pick any subset. Two deployers back this, both `isDevTree`-refusing, idempotent, backup-before-write:
   ...
   | `pulse` | the Pulse dashboard (`launchd` on macOS, Scheduled Task/Startup fallback on Windows, `:31337`) | optional |
